...
import { addContribution } from "@/lib/demo-activity"
import { mockContribute } from "@/lib/mock-api"
...
export function ContributeDataModal({ isOpen, onClose, model, campaignId }: ContributeDataModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await mockContribute(campaignId, firstFile?.name || 'data.pdf')
      
      const contribId = result.contributionId || "CONTRIB-" + Math.random().toString(36).substring(2, 10).toUpperCase()
      ...
      uploadedFiles.forEach((file) => {
        addContribution({
          ...
          fileName: file.name,
          estimatedEarnings: 50 + Math.floor(Math.random() * 100),
        })
      })
